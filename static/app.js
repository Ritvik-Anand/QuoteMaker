...
document.addEventListener('click', e => {
  if (!e.target.closest('.share-wrap')) closeAllShareMenus();
});
...
function shareViaWhatsApp(id, clientName) {
  if (!id) return;
  closeAllShareMenus();
  const label = (clientName || '').trim() || 'you';
  const text = `Here's the quote for ${label} autogenerated and sent via Slate\n${shareLinkFor(id)}`;
